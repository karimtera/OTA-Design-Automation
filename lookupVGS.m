...
if  mode == 1
    VGS = data.VGS;
    ratio = lookup1(data, ratio_string, 'VGS', VGS, 'VDS', par.VDS, 'VSB', par.VSB, 'L', par.L);
    ...
    VGS    = par.VGB - VSB;
	VDS    = par.VDB - VSB;
	ratio  = lookup1(data, ratio_string, 'VGS', VGS, 'VDS', VDS, 'VSB', VSB, 'L', ones(length(VSB),1)*par.L);
    idx    = isfinite(ratio);
    ratio  = ratio(idx);
    VGS    = VGS(idx);
end    
